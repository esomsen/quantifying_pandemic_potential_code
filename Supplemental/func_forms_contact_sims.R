...
H1N1.alt.s.vals <- c()
while (length(H1N1.alt.s.vals) < its*length(contact.nums)){
...
for (c in 1:length(contact.nums)){
  ...
  H1N1.Tcs <- append(H1N1.Tcs, mean(tmp.gen.time))
  print(paste("H1N1 rate", c, "done"))
  ## tidy environment
  rm(list=ls(pattern="^tmp"))
}
...
## use Metropolis-Hastings algorithm to accept or reject draws
H3N2.alt.s.vals <- c()
while (length(H3N2.alt.s.vals) < its*length(contact.nums)){
...
for (c in 1:length(contact.nums)){
  ...
  ## record Tc for this contact rate
  H3N2.Tcs <- append(H3N2.Tcs, mean(tmp.gen.time))
  print(paste("H3N2 rate", c, "done"))
  ## tidy environment
  rm(list=ls(pattern="^tmp"))
}


save.image(file="linear_contact_sims.RData")
...
## use Metropolis-Hastings algorithm to accept or reject draws
H1N1.alt.params <- matrix(data=NA, nrow=its*length(contact.nums), ncol=2)
colnames(H1N1.alt.params) <- c("h", "s")
while (is.na(H1N1.alt.params[its*length(contact.nums),1]) == TRUE){
  ...
  if (tmp.accept.ratio > runif(1, 0, 1)){
    H1N1.alt.params[which.min(!is.na(H1N1.alt.params[,1])),"h"] <- tmp.prop.h
    H1N1.alt.params[which.min(!is.na(H1N1.alt.params[,1])),"s"] <- tmp.prop.s
  }
  ## tidy environment
  rm(list=ls(pattern="^tmp"))
}
  ...
  for (i in 1:its){
    ...
    for (ferret in H1N1_recipient_names){
      ...
      for (t in tmp.contact.times){
        ...
        if (tmp.data[which(near(tmp.data$dpe, t)),"nw_titer"] >= H1N1.alt.params[[i*c,1]]){
          tmp.pr <- calculate_pr_contact_pos(AUC(x=c(0, exposure.length), y=rep(H1N1.alt.params[[i*c,2]], 2), method="trapezoid"))
...
## draw new param values for each simulation
## use Metropolis-Hastings algorithm to accept or reject draws
H3N2.alt.params <- matrix(data=NA, nrow=its*length(contact.nums), ncol=2)
colnames(H3N2.alt.params) <- c("h", "s")
while (is.na(H3N2.alt.params[its*length(contact.nums),1]) == TRUE){
  ...
  if (tmp.accept.ratio > runif(1, 0, 1)){
    H3N2.alt.params[which.min(!is.na(H3N2.alt.params[,1])),"h"] <- tmp.prop.h
    H3N2.alt.params[which.min(!is.na(H3N2.alt.params[,1])),"s"] <- tmp.prop.s
  }
  ## tidy environment
  rm(list=ls(pattern="^tmp"))
}
  ...
  for (i in 1:its){
    ...
    for (ferret in H3N2_recipient_names){
      ...
      for (t in tmp.contact.times){
        ## calculate a constant force of infection given 1 hour exposure
        ## if titer is above h, FOI = s (aka r in manuscript)
        if (tmp.data[which(near(tmp.data$dpe, t)),"nw_titer"] >= H3N2.alt.params[[i*c,1]]){
          tmp.pr <- calculate_pr_contact_pos(AUC(x=c(0, exposure.length), y=rep(H3N2.alt.params[[i*c,2]], 2), method="trapezoid"))
        } else { ## if titer < h, force of infection is 0
          tmp.pr <- 0
        }
        tmp.pr.transmission <- append(tmp.pr.transmission, tmp.pr)
...
save.image(file="threshold_contact_sims.RData")
...
## draw new param values for each simulation
## use Metropolis-Hastings algorithm to accept or reject draws
H1N1.alt.params <- matrix(data=NA, nrow=its*length(contact.nums), ncol=3)
...
## draw new param values for each simulation
## use Metropolis-Hastings algorithm to accept or reject draws
H3N2.alt.params <- matrix(data=NA, nrow=its*length(contact.nums), ncol=3)
...
save.image(file="hill_contact_sims.RData")