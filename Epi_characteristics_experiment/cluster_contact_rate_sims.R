...
for (ferret in H1N1_recipient_names){
  ...
  lambda_vals <- c()
  ## calculate a constant force of infection given exposure to each viral titer
  for (k in 1:length(data$nw_titer)){
    if (data[[k,"nw_titer"]] == LOD){
      ## if titer = LOD, we assume that the force of infection is 0
      lambda <- 0 
    } else { ## otherwise, calculate constant lambda for exposure
...
print("H3N2 transmission probs done")
