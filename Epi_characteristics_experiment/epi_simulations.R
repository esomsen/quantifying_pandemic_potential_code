...
## load in R files from contact rate simulations
...
ggplot(mean.R0s, aes(x=contact.nums, y=R0, color=Subtype, fill=Subtype)) +
  geom_col(position = "dodge") +
  scale_color_manual(values = c(plot_colors[[1]], plot_colors[[2]])) + 
  scale_fill_manual(values = c(plot_colors[[1]], plot_colors[[2]])) +
  guides(color="none", fill="none") +
  labs(x="Number of contacts", y=expression(paste("Mean ", R[0]))) +
  theme_light()
...
## calculate prob of extinction for each subtype at each num of contacts
H1N1.extinction.probs <- c()
H3N2.extinction.probs <- c()

for (c in 1:length(contact.nums)){
  H1N1 <- H1N1.R0s[c]
  H1N1.extinction.probs <- append(H1N1.extinction.probs, prob.extinction(H1N1, k))
  H3N2 <- H3N2.R0s[c]
  H3N2.extinction.probs <- append(H3N2.extinction.probs, prob.extinction(H3N2, k))
}
...
ggplot(extinction.probs, aes(x=contact.nums, y=1-prob.extinction, color=Subtype, fill=Subtype)) +
  geom_col(position="dodge") +
  scale_color_manual(values = c(plot_colors[[1]], plot_colors[[2]])) + 
  scale_fill_manual(values = c(plot_colors[[1]], plot_colors[[2]])) +
  guides(color="none", fill="none") +
